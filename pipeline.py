...
필요 API 키 (.env):
  ANTHROPIC_API_KEY=sk-ant-...       # Opus 4.6 (오케스트레이터)
...
def opus_orchestrate(user_input: str) -> dict:
    """
    Opus 4.6이 사용자 입력을 분석하여 전체 파이프라인 지시서를 생성합니다.

    Returns:
        dict with keys:
          - music_prompt: Lyria 3 Pro에 보낼 상세 영문 프롬프트
          - title_ko: YouTube 제목 (한국어)
          - title_en: YouTube 제목 (영어 서브)
          - description: YouTube 설명문
          - tags: 태그 리스트
          - visual_style: 비주얼 스타일 지시 (color_scheme, animation_type 등)
          - filename_base: 파일명 베이스
    """
    import anthropic

    client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)

    system_prompt = """당신은 AI 음악 프로덕션 총괄 프로듀서입니다.
...
6. 상업적 품질을 목표로 상세하게 기술"""
    ...
    message = client.messages.create(
        ...
        messages=[
            {"role": "user", "content": f"다음 요청으로 완전한 프로덕션 지시서를 만들어주세요:\n\n{user_input}"}
        ]
    )
